...
GO
CREATE OR ALTER FUNCTION [dbo].[F_PEB_0210] 
( 
...
(
	...
	TAKE_P_AMT		numeric(18),	-- 계획-생산성(매출)
	PROFIT_P_AMT	numeric(18),	-- 계획-생산성(이익)
	PHM_P_CNT		numeric(8,2),		-- 인원계획
	PAY_P_CNT		numeric(8,2),		-- 인원계획
	PAY_P_AMT		numeric(18),	-- 급여계획
...
--<DOCLINE>   RETURN      : 
--<DOCLINE>   COMMENT     : 
--<DOCLINE>   HISTORY     : 작성 lhs 2021.05.24
--<DOCLINE> ***************************************************************************
BEGIN
...
(
	...
	INSERT INTO @returnTable
		SELECT 
				...
				0 ORG_ID			, -- 부서
				NULL MON_CNT			, -- 개월수
				NULL TAKE_P_AMT		, -- 계획-생산성(매출)
				NULL PROFIT_P_AMT	, -- 계획-생산성(이익)
				NULL PHM_P_CNT		, -- 인원계획
				NULL PAY_P_CNT		, -- 인원계획
				NULL PAY_P_AMT		, -- 급여계획
	...
	SELECT --@av_company_cd
		...
		, ISNULL(dbo.F_FRM_ORM_ORG_NM(A.ORG_ID, 'KO', MAX(BASE_YMD), '1'),
		 ...
		 , ORG_ID
		 , T.MON_CNT
		 , SUM(TAKE_P_AMT  ) AS TAKE_P_AMT
		 , SUM(PROFIT_P_AMT) AS PROFIT_P_AMT
		 , SUM(PHM_P_CNT   ) / T.MON_CNT AS PHM_P_CNT
		 , SUM(PAY_P_CNT   ) / T.MON_CNT AS PAY_P_CNT
		 , SUM(PAY_P_AMT   ) AS PAY_P_AMT
	  ...
	  FROM (
		SELECT BASE_YM, BASE_YMD, NULL SUPER_ORG_ID, /* B.SUPER_ORG_ID,*/ A.ORG_ID, TAKE_P_AMT, PROFIT_P_AMT, PHM_P_CNT, PAY_P_CNT,
				   PAY_P_AMT, TAKE_AMT, PROFIT_AMT, PHM_CNT, PAY_CNT, PAY_AMT
			  FROM (SELECT BASE_YM, BASE_YMD, ORG_ID
						 , SUM(CASE WHEN PLAN_CD='10' THEN TAKE_AMT   ELSE 0 END) AS TAKE_P_AMT
						 , SUM(CASE WHEN PLAN_CD='10' THEN PROFIT_AMT ELSE 0 END) AS PROFIT_P_AMT
						 , SUM(CASE WHEN PLAN_CD='10' THEN PHM_CNT    ELSE 0 END) AS PHM_P_CNT
						 , SUM(CASE WHEN PLAN_CD='10' THEN PAY_CNT    ELSE 0 END) AS PAY_P_CNT
						 , SUM(CASE WHEN PLAN_CD='10' THEN PAY_AMT    ELSE 0 END) AS PAY_P_AMT
					  ...
					  SELECT BASE_YM, DBO.XF_LAST_DAY(BASE_YM + '01') BASE_YMD, ORG_ID, PLAN_CD
						 ...
						 WHERE 1=1
						   ...
						   AND (@prod_kind = '10' OR PLAN_CD = '20') -- 계획 및 전년 -- PLAN_CD-10-계획,20-실적
						UNION ALL
					  SELECT BASE_YM, DBO.XF_LAST_DAY(BASE_YM + '01') BASE_YMD, ORG_ID, PLAN_CD
					...
					-- 전년데이타만
					  ...
					  SELECT CONVERT(NVARCHAR(4),SUBSTRING(BASE_YM,1,4) + 1) + SUBSTRING(BASE_YM,5,2) BASE_YM
						 ...
						 WHERE 1=1
						   ...
						   AND BASE_YM >= CONVERT(NVARCHAR(4), SUBSTRING(@fr_month, 1, 4) - 1) + SUBSTRING(@fr_month, 5, 2)
						   AND BASE_YM <= CONVERT(NVARCHAR(4), SUBSTRING(@to_month, 1, 4) - 1) + SUBSTRING(@to_month, 5, 2)
						   AND (@prod_kind = '20' AND PLAN_CD = '20') -- 계획 및 전년
						   AND CASE WHEN VIEW_CD='50' THEN 'Y' ELSE @arb_inc_yn END = @arb_inc_yn-- 일용/도급포함
						   ) U
	...
	WITH H_ORG AS (
		...
		SELECT 
				...
				A.ORG_ID			, -- 부서
				A.MON_CNT			, -- 개월수
				A.TAKE_P_AMT		, -- 계획-생산성(매출)
				A.PROFIT_P_AMT	, -- 계획-생산성(이익)
				A.PHM_P_CNT		, -- 인원계획
				A.PAY_P_CNT		, -- 인원계획
				A.PAY_P_AMT		, -- 급여계획

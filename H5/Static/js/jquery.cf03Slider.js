...
// cf03Slider의 작동 방식을 direction이 horizontal인 경우에 대해 설명드리면(가로 슬라이드, direction이 vertical이면 세로 슬라이드)
...
;(function($, window, document, undefined) {
	...
	var pluginName = 'cf03Slider',
		defaults = {
			...
			nextEventType: null,		// 활용예) 모바일웹 개발할 때 터치 swipe(플리킹)으로 slider를 작동시키고 싶으면 이 자리에 적절한
										// 커스텀 이벤트 타입을 등록하고, 터치를 할 때 그 커스텀 이벤트를 cf03Slider를 실행시킬 엘리먼트에서 발생시키면 됨
	...
	Plugin.prototype.init = function() {
		...
		var slider = $(this.element),
			...
			$items = $container.find(options.item).not('.cf03slider_clone'),
		...
		$beforeItems.each(function() {
			$(this).addClass('cf03slider_clone');
		});
		...
		// 이전 버튼에 이벤트 발생시 실행
		$prevBtn
			.unbind(options.eventType + '.cf03Slider')
			.bind(options.eventType + '.cf03Slider', function() {
		...
		if (options.prevEventType) {
			slider
				.unbind(options.prevEventType + '.cf03Slider')
				.bind(options.prevEventType + '.cf03Slider', function() {
					go('prev', $container, marginType, itemSize, itemLength, options);
				});
		}
		...
		if (options.nextEventType) {
			slider
				.unbind(options.nextEventType + '.cf03Slider')
				.bind(options.nextEventType + '.cf03Slider', function() {
					go('next', $container, marginType, itemSize, itemLength, options);
				});
		}
	...
	// go 함수를 cf03Slider 인스턴스의 메서드로 만듬

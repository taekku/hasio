 BEGIN
	 ...
	 DECLARE --@v_source_company_cd NVARCHAR(100) = 'X'
		   ...
		   --, @v_start_option_key nvarchar(100) = 'PAY_BASE'-- 급여기준관리 2021.04.05
	...
	DECLARE mm CURSOR LOCAL FOR
		   WITH CTE_E AS (
		       SELECT K.*
		         ...
		         FROM [172.20.16.40].[dwehrdev_H5].[dbo].FRM_OPTION_ITEM K 
		       ...
		       SELECT B.* 
		         ...
		         FROM CTE_E A  
		              INNER JOIN [172.20.16.40].[dwehrdev_H5].[dbo].FRM_OPTION_ITEM B ON ( A.OPTION_ITEM_ID = B.PARENT_ITEM_ID 
		                                            AND A.COMPANY_CD = B.COMPANY_CD -- @v_source_company_cd 
													)
		   )
	...
	WHILE(@@FETCH_STATUS = 0)
		BEGIN
			...
			SELECT
			     ...
			     , CASE WHEN @n_parent_item_id IS NULL THEN NULL 
			            ELSE (
			                SELECT OPTION_ITEM_ID
			                 ...
			                 WHERE COMPANY_CD = @v_company_cd --T.COMPANY_CD -- @v_target_company_cd
			                   AND OPTION_KEY = (
			                       SELECT OPTION_KEY 
			                         FROM [172.20.16.40].[dwehrdev_H5].[dbo].FRM_OPTION_ITEM
			     ...
			     , @v_option_value                   -- 옵션 값
			     , @n_seq_order                      -- 정렬순서
			     , @v_company_cd -- T.COMPANY_CD -- @v_target_company_cd                     -- 인사영역코드
			     , @d_sta_ymd                        -- 시작일
			     , @d_end_ymd                        -- 종료일
			     , @n_mod_user_id                    -- 변경자ID
			     , GETDATE()                       -- 변경일시

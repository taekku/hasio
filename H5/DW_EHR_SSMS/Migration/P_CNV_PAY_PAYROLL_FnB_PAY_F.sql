...
-- 
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[P_CNV_PAY_PAYROLL_FnB_PAY_F]
...
BEGIN
	...
	set @v_s_table = 'CNV_PAY_DTL_SAP_FINAL'   -- As-Is Table
	...
	WHILE 1=1
		BEGIN
			...
			BEGIN TRY
				...
				EXEC @pay_ymd_id = P_CNV_PAY_PAY_YMD_SAP_PAY_F @an_log_h_id=@n_log_h_id, @av_company_cd=@av_company_cd,
				...
				DECLARE EMP_CUR CURSOR READ_ONLY FOR
					SELECT EMP_ID, MAX(CD_COST) AS CD_COST, MAX(ES_GRP) ES_GRP,
							CASE
								WHEN MAX(ES_GRP) IN ('11') THEN '001' -- 임원
								WHEN MAX(ES_GRP) IN ('21') THEN '010' -- 시급제
								WHEN MAX(ES_GRP) IN ('41') THEN '005' -- 월급제
								WHEN MAX(ES_GRP) IN ('51') THEN '002' -- 연봉제
								WHEN MAX(ES_GRP) IN ('61') THEN '010' -- 계약제
								WHEN MAX(ES_GRP) IN ('71') THEN '002' -- 전문제
								ELSE '002' END, -- @salary_type_cd
							CASE
								WHEN MAX(ES_GRP) IN ('11') THEN 'B' -- 임원
								WHEN MAX(ES_GRP) IN ('21') THEN 'T' -- 시급제
								WHEN MAX(ES_GRP) IN ('41') THEN 'M' -- 월급제
								WHEN MAX(ES_GRP) IN ('51') THEN 'Y' -- 연봉제
								WHEN MAX(ES_GRP) IN ('61') THEN 'C' -- 계약제
								WHEN MAX(ES_GRP) IN ('71') THEN 'Y' -- 전문제
								ELSE 'Y' END, -- @tp_calc_ins
							SUM(CASE WHEN TP_CODE = '1' THEN AMT ELSE 0 END) AS PSUM,
							SUM(CASE WHEN TP_CODE = '2' THEN AMT ELSE 0 END) AS DSUM
					  FROM CNV_PAY_DTL_SAP_FINAL A
					  JOIN CNV_PAY_ITEM B
...
--DROP INDEX CNV_PAY_DTL_SAP_01_IX01 ON CNV_PAY_DTL_SAP_01
--CREATE INDEX CNV_PAY_DTL_SAP_01_IX01 ON CNV_PAY_DTL_SAP_01 (DT_PROV, CD_PAYGP, SAP_KIND1, SAP_KIND2, EMP_ID, CD_ITEM)
				OPEN EMP_CUR
				WHILE 1=1
					BEGIN
						...
						BEGIN TRY
							...
							SELECT TOP 1 @pay_payroll_id,
									...
									dbo.F_ORM_ORG_BIZ(ISNULL(A.ORG_ID, EMP.ORG_ID), @dt_prov, 'PAY')	PAY_BIZ_CD,--	급여사업장코드
									dbo.F_ORM_ORG_BIZ(ISNULL(A.ORG_ID, EMP.ORG_ID), @dt_prov, 'PAY')	RES_BIZ_CD,--	지방세사업장코드
									ISNULL(A.ORG_ID, EMP.ORG_ID)	ORG_ID, --	발령부서ID
									ISNULL(A.ORG_ID, EMP.ORG_ID) PAY_ORG_ID, --	급여부서ID
									ISNULL(A.MGR_TYPE_CD, EMP.MGR_TYPE_CD)	MGR_TYPE_CD,-- 관리구분코드
									ISNULL(A.POS_CD, EMP.POS_CD)	POS_CD, --	직위코드[PHM_POS_CD]
									ISNULL(A.JOB_POSITION_CD, EMP.JOB_POSITION_CD)	JOB_POSITION_CD, --	직종코드
									ISNULL(A.DUTY_CD, EMP.DUTY_CD)	DUTY_CD, -- 직책코드[PHM_DUTY_CD]
									ISNULL(A.EMP_KIND_CD, EMP.EMP_KIND_CD)	EMP_KIND_CD, -- 근로구분코드[PHM_EMP_KIND_CD]
									...
									NULL	FILLDT, --	기표일
									ISNULL(A.POS_GRD_CD, EMP.POS_GRD_CD)	POS_GRD_CD, --	직급[PHM_POS_GRD_CD]
									ISNULL(A.YEARNUM_CD, EMP.YEARNUM_CD)	PAY_GRADE,-- 호봉코드 [PHM_YEARNUM_CD]
									NULL	DTM_TYPE, --	근태유형
									NULL	FILLNO, --	전표번호
									NULL	NOTICE, --	급여명세공지
								...
								FROM PHM_EMP EMP
								JOIN CAM_HISTORY A
								  ON EMP.COMPANY_CD = A.COMPANY_CD
								AND @dt_prov BETWEEN STA_YMD AND END_YMD
								WHERE EMP.COMPANY_CD = @t_company_cd

...
GO
-- =============================================
-- Description:	Conversion 급여공제내역
-- H_MONTH_DEDUCT => PAY_PAYROLL_DETAIL(공제항목)
-- =============================================
CREATE OR ALTER PROCEDURE P_CNV_PAY_PAYROLL_DETAIL_DEDUCT
...
BEGIN
	...
	set @v_s_table = 'H_MONTH_DEDUCT'   -- As-Is Table
		...
		BEGIN
			...
			BEGIN TRY
				...
				SELECT @cd_paygp = CD_PAYGP
				  FROM dwehrdev.dbo.H_MONTH_PAY_BONUS (NOLOCK)
				...
				SELECT @pay_item_cd = ITEM_CD, @nm_item = NM_ITEM
				 ...
				 WHERE COMPANY_CD = @t_company_cd
				   AND CD_ITEM = @cd_deduct
				   AND TP_CODE = '2' -- 공제항목코드
				IF @@ROWCOUNT < 1 OR ISNULL(@pay_item_cd,'') = '' --OR ISNULL(@pay_item_type_cd, '') = ''
				BEGIN
					IF @cd_deduct in ('226') -- ASIS에서 공제항목코드가 없음. I
						BEGIN
							SELECT @pay_item_cd = ITEM_CD, @nm_item = NM_ITEM
								 , @pay_item_type_cd = dbo.F_FRM_UNIT_STD_VALUE (@t_company_cd, 'KO', 'PAY', 'PAY_ITEM_CD_BASE',
										  NULL, NULL, NULL, NULL, NULL,
					...
					ELSE
						BEGIN
							-- *** 로그에 실패 메시지 저장 ***
							set @v_keys = ISNULL(CONVERT(nvarchar(100), @cd_company),'NULL')
								  + ',cd_deduct=' + ISNULL(CONVERT(nvarchar(100), @cd_deduct),'NULL')
								  + ',ym_pay=' + ISNULL(CONVERT(nvarchar(100), @ym_pay),'NULL')
								  + ',fg_supp=' + ISNULL(CONVERT(nvarchar(100), @fg_supp),'NULL')
								  + ',dt_prov=' + ISNULL(CONVERT(nvarchar(100), @dt_prov),'NULL')
								  + ',no_person=' + ISNULL(CONVERT(nvarchar(100), @no_person),'NULL')
				...
				BEGIN CATCH
					...
					IF @n_err_cod = 2627 -- 중복키
						BEGIN
							UPDATE A
							   SET BASE_MON = BASE_MON + B.AMT_DEDUCT
								 , CAL_MON = CAL_MON + B.AMT_DEDUCT
							  FROM PAY_PAYROLL_DETAIL A
							  JOIN (SELECT AMT_DEDUCT FROM dwehrdev.dbo.H_MONTH_DEDUCT A WITH (NOLOCK)
									 WHERE A.CD_COMPANY = @s_company_cd
									   AND A.YM_PAY = @ym_pay
									   AND A.FG_SUPP = @fg_supp
									   AND A.DT_PROV = @dt_prov
									   AND A.NO_PERSON = @no_person
									   AND A.CD_DEDUCT = @cd_deduct) B
								ON 1=1

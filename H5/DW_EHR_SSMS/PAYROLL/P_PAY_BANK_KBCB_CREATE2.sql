...
GO
CREATE PROCEDURE [dbo].[P_PAY_BANK_KBCB_CREATE2]
...
AS    
   /*
   *   <DOCLINE> ***************************************************************************
   *   <DOCLINE>   TITLE       : KB Cyber Branch 이체파일 생성
   *   <DOCLINE>   PROJECT     : BAYER
   *   <DOCLINE>   AUTHOR      : 임택구
   *   <DOCLINE>   PROGRAM_ID  : P_PAY_BANK_KBCB_CREATE
...
BEGIN
    ...
    DECLARE
		...
		@n_pay_payroll_id	numeric(38),
		@n_seq			numeric(8) = 0,
        @n_cnt			numeric(8) = 0

    /* 기본변수 초기값 셋팅*/
    SET @av_ret_code = 'SUCCESS!'
    SET @v_program_id = 'P_PAY_BANK_KBCB_CREATE'/* 현재 프로시져의 영문명*/
    SET @v_program_nm = 'KB CyberBank이체파일 생성'/* 현재 프로시져의 한글문명*/
          ...
          BEGIN TRY
				...
				INSERT CB2_PAY_REQ_D( SITE_CD, --	사업장코드
                           ...
                           SELECT @av_site_cd AS SITE_CD, -- 사업장코드
								  ...
								  +
								  CASE WHEN @av_company_cd IN ('C','T') THEN '>>급여>' + @v_pay_group ELSE '>>급여이체' END AS REMARK,

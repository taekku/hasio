...
-- 
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[P_CNV_PAY_PAYROLL_ONE]
					...
					------------
						INSERT INTO PAY_PAYROLL_DETAIL(
							...
							SELECT  NEXT VALUE FOR S_PAY_SEQUENCE AS PAY_PAYROLL_DETAIL_ID, --	급여상세내역ID
									...
									case when @s_company_cd='F' and @cd_paygp='F21' and A.CD_ALLOW='1001' then 'P132' else B.ITEM_CD end	PAY_ITEM_CD, --	급여항목코드
									SUM(A.AMT_ALLOW_2)	BASE_MON, --	기준금액
									SUM(A.AMT_ALLOW)	CAL_MON, --	계산금액
									0	FOREIGN_BASE_MON, --	외화기준금액
									0	FOREIGN_CAL_MON, --	외화계산금액
									dbo.F_FRM_UNIT_STD_VALUE (@t_company_cd, 'KO', 'PAY', 'PAY_ITEM_CD_BASE',
									  NULL, NULL, NULL, NULL, NULL,
									  case when @s_company_cd='F' and @cd_paygp='F21' and A.CD_ALLOW='1001' then 'P132' else B.ITEM_CD end, NULL, NULL, NULL, NULL,
									  getdATE(),

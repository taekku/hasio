...
GO
CREATE OR ALTER PROCEDURE dbo.P_PBT_AT_DEBIS_IF_STP_COM(
...
AS
    ...
    --<DOCLINE>   AUTHOR      : 임택구
    --<DOCLINE>   PROGRAM_ID  : P_PBT_AT_DEBIS_IF
    --<DOCLINE>   ARGUMENT    : 
...
--IF @av_company_cd = 'B'
--	BEGIN
--		EXECUTE dbo.P_PBT_AT_DEBIS_B_IF_STP_COM
--					  @av_company_cd
--					, @av_hrtype_gbn
--					, @av_bill_gbn
--					, @av_pay_ym
...
BEGIN TRY
    ...
    SET @v_program_id   = 'P_PBT_AT_DEBIS_IF_STP_COM'
    ...
    BEGIN
		...
		IF @@error <> 0
			BEGIN
				...
				SET @v_error_code = 'P_PBT_AT_DEBIS_IF_STP_COM';
				 ...
				 IF @V_ITEM_CD = 'C'
					BEGIN
						...
						IF @V_INCCOSTDPT_CNT = 1
							BEGIN
								SET @V_INCCOSTDPT_STR = @V_INCCOSTDPT_STR + ' AND ((EMP.ACC_CD BETWEEN ''' + @V_INCITEM_FR + ''' AND ''' + @V_INCITEM_TO + ''')' + CHAR(13);
							END
						ELSE
							BEGIN
								SET @V_INCCOSTDPT_STR = @V_INCCOSTDPT_STR + ' OR (EMP.ACC_CD BETWEEN ''' + @V_INCITEM_FR + ''' AND ''' + @V_INCITEM_TO + ''')' + CHAR(13);
							END
					END    
				 --직급일 경우
				 IF @V_ITEM_CD = 'D'
					BEGIN
						...
						IF @V_INCJIKGUB_CNT = 1
							BEGIN
								SET @V_INCJIKGUB_STR = @V_INCJIKGUB_STR + ' AND ((ISNULL(dbo.F_FRM_CODE_NM(EMP.COMPANY_CD,''KO'',''PBT_POS_GRD_CD'', EMP.POS_GRD_CD, NTS.PROC_DATE,''M''),'''') BETWEEN ''' + @V_INCITEM_FR + ''' AND ''' + @V_INCITEM_TO + ''')';
							END
						ELSE
							BEGIN
                 				SET @V_INCJIKGUB_STR = @V_INCJIKGUB_STR + ' OR (ISNULL(dbo.F_FRM_CODE_NM(EMP.COMPANY_CD,''KO'',''PBT_POS_GRD_CD'', EMP.POS_GRD_CD, NTS.PROC_DATE,''M''),'''') BETWEEN ''' + @V_INCITEM_FR + ''' AND ''' + @V_INCITEM_TO + ''')';
							END
					END
					...
					BEGIN
						...
						ELSE IF @V_INCITEM = 'P99050'  --(공제총액)
							BEGIN
								...
								--					 ' AND YM_PAY = ''' + @P_YYYYMM + '''' + ' AND FG_SUPP = ''' + @P_PAY_CD + ''''; 
								-- 신협출자금을 제외하고 있는데 실제 공제총액을 계산하는 계정코드는 없음
				...
				IF @V_AGGR_GBN = 'A1'
					BEGIN
						SET @V_RESULT_STR = ' SELECT '''' AS SABUN' +
						                    ' , ACC_CD AS COSTDPT_CD' +
											' ,	SUM(INSU_AMT) AS INSU_AMT ' +
											' , SUM(CAL_MON) AS CAL_MON ' +
											' FROM (SELECT EMP_ID' +
											'            , SUM( ISNULL(INSU_MON,0) + ISNULL(ADJ_INSU_AMT, 0) ) INSU_AMT ' +
											'		  FROM STP_NOTICE ' +
											'		 WHERE COMPANY_CD = ''' + @av_company_cd + '''' +
											'          AND BASE_YM = ''' + @av_pay_ym + '''' +
											'		 GROUP BY EMP_ID) NTC ' +
											'  LEFT JOIN ( SELECT PAY.EMP_ID, ' +
											'                     SUM( DTL.CAL_MON ) AS CAL_MON ' +
											...
											--'                 AND DTL.PAY_ITEM_CD IN ( ' + @V_ALLLOW + ' ) '+
											                      @V_ALOW_STR +
											'               WHERE YMD.PAY_YM = ''' + @av_pay_ym + '''' +
											'				  AND YMD.CLOSE_YN = ''Y''' +
											'               GROUP BY PAY.EMP_ID ' +
											'                   ) PAY ' +
											'    ON NTC.EMP_ID = PAY.EMP_ID' +
											' INNER JOIN (SELECT COMPANY_CD, LOCALE_CD, EMP_ID, EMP_NO ' +
						                    '				   , dbo.F_PAY_GET_COST(''' + @av_company_cd + ''', EMP_ID, ORG_ID, ''' + FORMAT(@ad_proc_date,'yyyyMMdd') + ''', ''1'') ACC_CD' +
											'				   , ORG_ID ' +
											'				   , EMP_KIND_CD ' +
											'				FROM VI_FRM_PHM_EMP) EMP' +
											'         ON NTC.EMP_ID = EMP.EMP_ID' +
											'        AND EMP.LOCALE_CD=''KO''' +
											' INNER JOIN PHM_PRIVATE PRI' +
											'         ON NTC.EMP_ID = PRI.EMP_ID' +
											'        AND ''' + FORMAT(@ad_proc_date, 'yyyyMMdd') + ''' BETWEEN PRI.STA_YMD AND PRI.END_YMD' +
											' WHERE PRI.FROM_TYPE_CD = ''' + @av_hrtype_gbn + '''' + --여기
											'   AND ISNULL(PAY.CAL_MON, 0) = 0 ' +
											 CHAR(13) +
											        @V_INCITEM_STR +
				...
				ELSE IF @V_AGGR_GBN = 'A2'
					BEGIN
						SET @V_RESULT_STR = ' SELECT '''' AS SABUN' +
											...
											' , SUM(CAL_MON) AS CAL_MON ' +
											' FROM (SELECT EMP_ID' +
											'            , ''' + FORMAT(@ad_proc_date, 'yyyyMMdd') + ''' as PROC_DATE' +
											'            , SUM( ISNULL(INSU_MON,0) + ISNULL(ADJ_INSU_AMT, 0) ) INSU_AMT ' +
											'		  FROM STP_NOTICE ' +
											'		 WHERE COMPANY_CD = ''' + @av_company_cd + '''' +
											'          AND BASE_YM = ''' + @av_pay_ym + '''' +
                 ...
                 -- 적요(앞에 매달 전표생성되는 월을 표시)
					...
					IF @V_AGGR_GBN = 'A3'
						BEGIN  
							...
							SET @OPENQUERY = @OPENQUERY + 'LEFT OUTER JOIN FRM_CODE C ON C.COMPANY_CD =''X'' AND C.CD_KIND = ''PBT_CUST_CD'' AND C.MAIN_CD = B.CLNT_NO'
							-- TEST

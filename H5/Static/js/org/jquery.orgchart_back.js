...
(function($) {
	...
	$(".in_photo1").on("click", function(e) {   // 사진
		popup_org_infor($(this),1);
	});
	
	$(".in_content1").on("click", function(e) {   // 기타
		popup_org_infor($(this),2);
	});
	$(".out_content").on("click", function(e) {  		
		popup_org_infor($(this),1);
	});
	$(".in_content2").on("click", function(e) {   
		popup_org_infor($(this),1);
	});
	////////////////////////  기구조직도에서도 사용
	$(".phm_div1").on("click", function(e) {   // 사람 이름 
		popup_org_infor($(this), 1);
	});
	$(".small_div1").on("click", function(e) {   // org_popup이 생성
		popup_org_infor($(this),2);
	});
	$(".temp1_org_name").on("click", function(e) {   // org_popup이 생성
		popup_org_infor($(this),2);
	});
	$(".temp1_phm_div").on("click", function(e) {   // 사람 이름 
		popup_org_infor($(this), 1);
	...
	$(".cross_div").on("click", function(e) {
		var cross_div_id = $(this).attr("id");
		var replace_id = cross_div_id.replace("change_", "");        // cross_div에서 가지고온 아이디를 잘른다.
		var $this = $("#sell_id_" + replace_id);   
		var $row = $this.closest("tr");
		var text = $('#' + cross_div_id).html();
		if ($row.next("tr").is(":visible")) {     // 클릭했을때, 접힌다.
			...
			if($("#org_type").val()=="1" ){  // org_type가 1이면 하위노드의 차트를 그리지 않는다.
				...
				//alert("4");
				CreateChildChart(replace_id);  // 열릴때, 하위 레벨에 있는 노드에 차트를 그린다.
			}
		}
	});
	...
	function buildNode($node, $appendTo, level, opts, settings) {
		...
		if (settings['div_template'] === "1") { // temp1일 경우에 화상조직도가 출력된다.
			...
			var temp1_org_name = $("<div name='temp1_org_name_pro"+chart_org_id+"'>").addClass("temp1_org_name").addClass(bg_class_nm); //  부서명이 출력되는 부분
			...
			var small_div = $("<div name='small_div_pro"+chart_org_id+"'>").addClass("small_div1").addClass(bg_class_nm).text(opts.nodeText($node)); //  부서명이 출력되는 부분
			...
			if(pos_emp == "[]"){
				//pos_emp = pos_emp.replace("[", "");
				//pos_emp = pos_emp.replace("]", "-");
				var phm_div = $("<div name='phm_div_pro"+org_emp_id+" '>").addClass("phm_div1").text("");
			}else if(pos_emp.indexOf("[" && "]")){
				
				pos_emp = pos_emp.replace("[", "");
				pos_emp = pos_emp.replace("]", "&nbsp;");
				var phm_div = $("<div name='phm_div_pro"+org_emp_id+" '>").addClass("phm_div1").html(pos_emp);   // 이름과 직책이 나오게 된다,
			}
			
			$nodeDiv.append(phm_div);

			var out_content=$("<div id='out_con_"+chart_org_id+"' name='out_con_"+org_emp_id+"'>").addClass("out_content");	
			$nodeDiv.append(out_content);

			var pie_plus= pie_chart_draw(chart_org_id);
			$nodeDiv.append(pie_plus);
			...
			if (settings['pie_chart'] == "true") {
				pie_chart_draw(chart_org_id);
			}
...
function pie_chart_draw(chart_org_id){             // 파이차트를 그리는 함수 
	$.post("/orm/web/pie_chartData.jsp", "cust_col1="+$("#cust_col1").val() + "&chart_org_id=" + chart_org_id+ "&base_ymd=" + $("#base_ymd").val()).done(
		function (strText) {
			if ($.trim(strText) == "NOT") {
				//alert("아무것도없어요!");
...
//						actual = actual+parseInt(data['data1'].chart_data[0].actual);
						...
						budget = parseInt(data['data1'].chart_data[0].budget);
						actual = parseInt(data['data1'].chart_data[0].actual);
						}else{
						color1 = "black";
					}
...
//						actual = actual+parseInt(data['data1'].chart_data[1].actual);
						...
						budget = parseInt(data['data1'].chart_data[1].budget);
						actual = parseInt(data['data1'].chart_data[1].actual);
						}else {
						color2 = "black";
					}
...
//						actual = actual+parseInt(data['data1'].chart_data[2].actual);
						...
						budget = parseInt(data['data1'].chart_data[2].budget);
						actual = parseInt(data['data1'].chart_data[2].actual);
					} else {
						color3 = "black";
					}
...
//						actual = actual+parseInt(data['data1'].chart_data[3].actual);
						...
						budget = parseInt(data['data1'].chart_data[3].budget);
						actual = parseInt(data['data1'].chart_data[3].actual);
					} else {
						color4 = "black";
					}
...
//						actual = actual+parseInt(data['data1'].chart_data[4].actual);
						...
						budget = parseInt(data['data1'].chart_data[4].budget);
						actual = parseInt(data['data1'].chart_data[4].actual);
					} else {
						color5 = "black";
					}
					
			
					$("#out_con_"+chart_org_id).html("<tr><td style='text-align: left; font-size: x-small;'>Budget :</td><td style='text-align: right; font-size: x-small;'>"+budget+"명</td></tr><tr><td style='text-align: left; font-size: x-small;'>&nbspActual :</td><td style='text-align: right; font-size: x-small;'>"+actual+"명</td></tr>");	
...
function CreateChildChart(replace_id){
	...
	//var tdId = tr.children("td").children("table").children("tbody").children("tr").children("td").children("div");

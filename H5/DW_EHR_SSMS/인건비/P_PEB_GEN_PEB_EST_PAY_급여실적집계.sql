  ...
  -- 기존자료 삭제
	...
	BEGIN TRY
		...
		SELECT YMD.PAY_YM BASE_YM
			 , PAY.ORG_ID
			 , dbo.F_PEB_GET_VIEW_CD('인건비', PAY.POS_CD, PAY.POS_GRD_CD, PAY.DUTY_CD, PAY.JOB_POSITION_CD) AS VIEW_CD
			 , COUNT(DISTINCT PAY.EMP_ID) AS PHM_CNT
			 , COUNT(DISTINCT PAY.EMP_ID) AS PAY_CNT
			 , SUM(CASE WHEN PEB.EST_TYPE_CD = '10' THEN DTL.CAL_MON
						ELSE DTL.CAL_MON END) PAY_AMT
			 , SUM(CASE WHEN PEB.EST_TYPE_CD = '10' THEN DTL.CAL_MON
						ELSE 0 END) PAY_ETC_AMT
		  ...
		  INNER JOIN (select KEY_CD1 ITEM_CD, HIS.STA_YMD, HIS.END_YMD
		 ...
		 GROUP BY YMD.PAY_YM, PAY.ORG_ID, dbo.F_PEB_GET_VIEW_CD('인건비', PAY.POS_CD, PAY.POS_GRD_CD, PAY.DUTY_CD, PAY.JOB_POSITION_CD)
----------------
-- 일용직급여 시작
----------------
		INSERT INTO @TEMP_EST_PAY(
...
----------------
		INSERT INTO PEB_EST_PAY(
		 ...
		 IF @@ROWCOUNT < 1
		 BEGIN
			SET @av_ret_message = dbo.F_FRM_ERRMSG( '인건비실적 집계중 에러[ERR]',

...
BEGIN
	...
	IF @@ROWCOUNT < 1
		BEGIN
				--SET @av_ret_code    = 'FAILURE!'
				--SET @av_ret_message = dbo.F_FRM_ERRMSG('연봉 인상율 읽어오기 에러[ERR]', @v_program_id,  120,  NULL, NULL);
				--RETURN
		...
		BEGIN
			...
			IF @n_year_limit IS NOT NULL AND DATEPART(YEAR, GetDate()) - DATEPART(YEAR, @POS_GRD_YMD) > @n_year_limit
				...
				select @v_prm_appl_ym = dbo.F_FRM_UNIT_STD_VALUE (@av_company_cd, @av_locale_cd, 'PEB', 'PEB_PROM_MONTH',
					...
					--IF @EMP_NO = '20130049'
					--print '승진대상1:' + @EMP_NO + ':' + ISNULL(@POS_GRD_CD,'NULL') + ':' + ISNULL(@v_prm_appl_ym, 'NULL')
			...
			BEGIN Try
				...
				SELECT NEXT VALUE FOR S_PEB_SEQUENCE AS PEB_CNM_CNT_ID,
						...
						--dbo.XF_CEIL(CNM.CNT_SALARY * @n_up_rate / 12, -1) * 12	AS CNT_SALARY, --	연간고정급
						--dbo.XF_CEIL(CNM.CNT_SALARY * @n_up_rate / 12, -1)		AS BASE_SALARY, --	기본급
				 ...
				 IF @d_end_ymd IS NOT NULL -- 승진
					BEGIN
						...
						SELECT NEXT VALUE FOR S_PEB_SEQUENCE AS PEB_CNM_CNT_ID,
								...
								'29991231' END_YMD, --	계약종료일자
								CNM.PRE_END_YMD	PRE_END_YMD, --	계약종료일
								@v_next_pos_grd_cd	PAY_POS_GRD_CD, --	급여직급코드[PAY_POS_GRD_CD]
								CNM.PAY_GRADE	PAY_GRADE, --	급여호봉[PAY_GRADE]
								CNM.PAY_JOP_CD	PAY_JOP_CD, --	급여직군[PAY_JOP_CD]
								CNM.CNT_SALARY	OLD_CNT_SALARY, --	이전고정급
								--dbo.XF_CEIL(CNM.CNT_SALARY * @n_up_rate / 12, -1) * 12	AS CNT_SALARY, --	연간고정급
								--dbo.XF_CEIL(CNM.CNT_SALARY * @n_up_rate / 12, -1)		AS BASE_SALARY, --	기본급
								@n_pos_grd_base_amt	AS CNT_SALARY, --	연간고정급
								dbo.XF_CEIL(@n_pos_grd_base_amt / 12, -1)	AS BASE_SALARY, --	기본급
								CNM.BP01	BP01, --	보전금액
								CNM.BP02	BP02, --	고정(연장)
								CNM.BP03	BP03, --	고정(야간)
								CNM.BP04	BP04, --	고정(휴일)
	...
	IF @n_up_rate IS NOT NULL
		BEGIN
			PRINT '연봉인상율 반영'
			--@n_up_rate = (PEB_RATE / 100.0) + 1 -- 연봉인상율
		    --, @v_peb_mm = A.PEB_YM -- 연봉인상월
				...
				BEGIN
					...
					ELSE
						BEGIN
							PRINT '쪼개기 안함'
							UPDATE PEB_CNM_CNT
							   SET CNT_SALARY = dbo.XF_CEIL(CNT_SALARY * @n_up_rate, -1) --	연간고정급
								 , BASE_SALARY = dbo.XF_CEIL(BASE_SALARY * @n_up_rate, -1) --	기본급
							 WHERE PEB_CNM_CNT_ID = @PEB_CNM_CNT_ID
						END
					FETCH NEXT FROM CUR_PEB_CNM_CNT
								INTO @PEB_CNM_CNT_ID, @d_cnm_sta_ymd, @d_cnm_end_ymd

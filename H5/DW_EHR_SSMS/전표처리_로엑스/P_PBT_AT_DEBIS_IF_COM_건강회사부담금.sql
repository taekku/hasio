...
GO
CREATE OR ALTER PROCEDURE dbo.P_PBT_AT_DEBIS_IF_COM(
...
IF @av_company_cd = 'B'
	BEGIN
		EXECUTE dbo.P_PBT_AT_DEBIS_B_IF_COM
					  @av_company_cd
					, @av_hrtype_gbn
					, @av_bill_gbn
					, @av_pay_ym
...
BEGIN TRY
    ...
    SET @v_program_id   = 'P_PBT_AT_DEBIS_IF_COM'
    ...
    BEGIN
		...
		IF @@error <> 0
			BEGIN
				...
				SET @v_error_code = 'P_PBT_AT_DEBIS_IF';
				...
				IF @V_AGGR_GBN = 'A1'
					BEGIN
						SET @V_RESULT_STR = ' SELECT '''' AS SABUN' +
											...
											' FROM (SELECT EMP_ID' +
											'            , SUM( INSU_AMT + JUNG_INSU_AMT + LONG_INSU_AMT + JUNG_LONG_AMT) INSU_AMT ' +
											'		  FROM NHS_NOTICE ' +

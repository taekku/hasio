...
public class KBDepositTransferCommand extends AbsBusinessCommand {
...
//			DataSource ds = (DataSource) ctx.lookup("java:comp/env/jdbc/cbpay");
	...
	protected ResponseContext executeCyberBranch(IRequestContext requestContext) throws CommandExecuteException {
		...
		IListBaseMessage paramMessage = (IListBaseMessage) requestContext.getMessage( "ME_PAY0089_01_01" );
		...
		if ( items.hasNext()) {
			...
			pay_ym 	    = (String) itemOne.getElement("pay_ym");
...
*/
		...
		try {
			...
			invoker = new SQLInvoker( "PAY0089_01_R01" , paramMap );
			...
			if( company_cd.equals("C") || company_cd.equals("T") ) {
				...
				invoker = new SQLInvoker( "PAY0089_01_D01" , paramMap );
				invoker.doService();
			}
			...
			try {
				...
				if(rs.next()){
					...
					CommandExecuteException cee = new CommandExecuteException("파일순번없음1.");
					throw cee;
				}
				rs.close();
			} catch(CommandExecuteException cee) {
			...
			paramMap.put("give_ymd"	    , pay_ymd );
			paramMap.put("pay_group"	, pay_group );;
			invoker = new SQLInvoker( "PAY0089_01_R02" , paramMap );
			result 	= (GridResult) invoker.doService();

			HashMap<String, Object> detailMap;
			String in_bank_cd, in_acct_no, real_amt, pre_reci_man;
			String remark;
			String trans_ymd = null, trans_hhmmss = null;
			...
			while( result.next() ) {
				...
				tot_amt += Long.parseLong(real_amt);
				...
				detailMap.put("erp_rec_no", pay_ymd);
				...
				if( company_cd.equals("C") || company_cd.equals("T") ) {
					invoker = new SQLInvoker( "PAY0089_01_I03" , detailMap );
					invoker.doService();
				}
			}

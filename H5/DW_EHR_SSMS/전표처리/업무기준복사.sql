BEGIN
	 DECLARE @v_source_company_cd NVARCHAR(100) = 'X'
	       , @v_target_company_cd NVARCHAR(100) = 'B'
		   , @v_unit_cd NVARCHAR(100) = 'PAY'
		   , @v_std_kind	NVARCHAR(100) = 'PAY_PBT_BILL,PAY_PBT_CUST,PAY_PBT_HRTYPE'
...
WHERE FRM_UNIT_STD_MGR_ID IN (
															...
															WHERE COMPANY_CD = @v_target_company_cd
...
DELETE FROM FRM_UNIT_STD_MGR WHERE COMPANY_CD = @v_target_company_cd
							AND UNIT_CD = @v_unit_cd
							AND STD_KIND IN (SELECT ITEMS FROM dbo.fn_split_array(@v_std_kind,','))
